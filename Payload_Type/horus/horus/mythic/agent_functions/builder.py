...
class HorusAgent(PayloadType):
    ...
    translation_container = "HorusTranslator" 
    build_parameters = [
        BuildParameter(
            name="output",
    ...
    agent_path = pathlib.Path(".") / "horus"
    agent_icon_path = agent_path / "agent_functions" / "horus.png"
    ...
    async def build(self) -> BuildResponse:
        ...
        with open(agent_build_path.name+"/horus/core/include/Config.h", "r+") as f:
            content = f.read()
        ...
        command = "make -C {} exe".format(agent_build_path.name+"/horus")
        ...
        if stderr:
            stdout_err += f'[stderr]\n{stderr.decode()}' + "\n" + command
        output_path = f"{agent_build_path.name}/horus/build/horus.exe"
