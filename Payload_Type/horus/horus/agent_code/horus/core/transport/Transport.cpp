...
Parser* makeHTTPRequest(PBYTE bufferIn, UINT32 bufferLen)
{
	...
	if (horusConfig->isProxyEnabled && horusConfig->proxyURL)
	{
		accessType = WINHTTP_ACCESS_TYPE_NAMED_PROXY;
		httpProxy = horusConfig->proxyURL;
	...
	if (horusConfig->isSSL)
		httpFlags |= WINHTTP_FLAG_SECURE;


	hSession = WinHttpOpen(horusConfig->userAgent, accessType, httpProxy, noProxyBypass, proxyFlag);
	if (!hSession)
		return NULL;

	hConnect = WinHttpConnect(hSession, horusConfig->hostName, horusConfig->httpPort, 0);
	if (!hConnect)
		return NULL;

	hRequest = WinHttpOpenRequest(hConnect, horusConfig->httpMethod, horusConfig->endPoint, NULL, NULL, NULL, httpFlags);
	if (!hRequest)
		return NULL;
	...
	if (WinHttpGetIEProxyConfigForCurrentUser(&proxyConfig))
	{
		...
		else if ((proxyConfig.lpszAutoConfigUrl != NULL && lstrlenW(proxyConfig.lpszAutoConfigUrl) != 0) || proxyConfig.fAutoDetect == TRUE)
		{
			...
			HttpUrlLen = (15 + lstrlenW(horusConfig->hostName) + lstrlenW(horusConfig->endPoint));
			HttpUrl = (LPWSTR)LocalAlloc(LPTR, HttpUrlLen * sizeof(WCHAR));

			WCHAR fullHttpScheme[20] = L"%ws://%ws:%lu/%ws";

			swprintf_s(HttpUrl, HttpUrlLen, fullHttpScheme, L"http", horusConfig->hostName, horusConfig->httpPort, horusConfig->endPoint);
